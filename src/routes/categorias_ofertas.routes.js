...
import { categorias_ofertas_services } from "../services/categorias_ofertas.service.js";
...
router.get("/", async (req, res, next) => {
    try {
        const response = await categorias_ofertas_services.getAll();
...
router.post("/", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await categorias_ofertas_services.createCategoria(req.body);
        return res.json(response);
    } catch (error) {
        next(error)
    }
...
router.put("/:id", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await categorias_ofertas_services.editCategoria(req.params.id, req.body);
        return res.json(response);
    } catch (error) {
        next(error)
    }
...
router.delete("/:id", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await categorias_ofertas_services.deleteCategoria(req.params.id);
        return res.json(response);
    } catch (error) {
        next(error)
    }
...
const categorias_ofertas_router = { router }
export { categorias_ofertas_router }
