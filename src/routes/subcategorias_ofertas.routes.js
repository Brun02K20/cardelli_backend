...
import { subcategorias_ofertas_services } from "../services/subcategorias_ofertas.service.js";
...
router.get("/", async (req, res, next) => {
    try {
        const response = await subcategorias_ofertas_services.getAll();
...
router.post("/", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await subcategorias_ofertas_services.createSubCategoria(req.body);
        return res.json(response);
    } catch (error) {
        next(error)
    }
...
router.put("/:id", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await subcategorias_ofertas_services.editSubCategoria(req.params.id, req.body);
        return res.json(response);
    } catch (error) {
        next(error)
    }
...
router.delete("/:id", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await subcategorias_ofertas_services.deleteSubCategoria(req.params.id);
        return res.json(response);
    } catch (error) {
        console.log(error)
        next(error)
...
const subcategorias_ofertas_router = { router }
export { subcategorias_ofertas_router }